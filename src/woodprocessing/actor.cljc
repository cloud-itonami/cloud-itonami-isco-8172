(ns woodprocessing.actor
  "WoodProcessingOperationsActor — the ISCO-08 8172 community woodprocessing actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one woodprocessing operation request
  ...
  The unconditional invariant: the WoodProcessingOperationsAdvisor can never
  directly commit a record the WoodProcessingOperationsGovernor refuses —
  every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [woodprocessing.advisor :as advisor]
            [woodprocessing.governor :as governor]
            [woodprocessing.store :as store]))

(defn build-graph
  "Build a compiled WoodProcessingOperationsActor graph. `store` implements
  `woodprocessing.store/Store`. `advisor` implements
  `woodprocessing.advisor/Advisor` (defaults to `mock-advisor`).
